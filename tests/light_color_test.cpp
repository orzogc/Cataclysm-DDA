...
static const dimension_id dimension_nether( "nether" );
...
TEST_CASE( "no_dawn_dusk_tint_in_alternate_dimension", "[light_color][dawn_dusk]" )
{
    ...
    const light_color_rgb default_dim = dawn_dusk_color_for_lightmap( dimension_default );
    CHECK( default_dim.is_colored() );
    CHECK( default_dim.r > 0.0f );

    // Alternate dimension: no tint
    const light_color_rgb nether = dawn_dusk_color_for_lightmap( dimension_nether );
